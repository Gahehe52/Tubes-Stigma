...
class DensityBot(BaseLogic):
    def __init__(self):
        ...
        self.last_teleporter_used: Optional[Position] = None  # prevent looping
    ...
    def next_move(self, board_bot: GameObject, board: Board) -> Tuple[int, int]:
        ...
        tele1, tele2 = (teleporters + [None, None])[:2]  # pad to avoid index error
        ...
        diamonds_list = sorted(
            diamonds,
            key=lambda d: (d.properties.points / max(1, distance_with_teleport(pos, d.position))),
            reverse=True
        )

        target = diamonds_list[0]
